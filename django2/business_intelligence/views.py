...
from .models import Customer, CustomerOrder, Product, Time, SalesFact, Location, SalesByLocation
...
SALES_LOCATION_PATH = os.path.join(BASE_DIR, 'dags2/fact_sales_by_location.csv')
LOCATION_DIM_PATH = os.path.join(BASE_DIR, 'dags2/dim_location.csv')
...
def load_etl_data(request):
    ...
    # Load fact_sales
    fact_sales_df = pd.read_csv(FACT_PATH)
    for _, row in fact_sales_df.iterrows():
        try:
            order = CustomerOrder.objects.get(order_number=row['ORDERNUMBER'])
            product = Product.objects.get(product_code=row['PRODUCTCODE'])
            order_date = Time.objects.get(order_date=row['ORDERDATE'])
            SalesFact.objects.update_or_create(
                order=order,
                product=product,
        ...
        except (CustomerOrder.DoesNotExist, Product.DoesNotExist, Time.DoesNotExist) as e:
            print(f"Missing reference in fact_sales: {e}")

    # Load dim_location
    location_df = pd.read_csv(LOCATION_DIM_PATH)
    ...
    # Load fact_sales_by_location
    location_sales_df = pd.read_csv(SALES_LOCATION_PATH)
    for _, row in location_sales_df.iterrows():
        SalesByLocation.objects.update_or_create(
            order_number=row['ORDERNUMBER'],
            customer_name=row['CUSTOMERNAME'],
            order_date=row['ORDERDATE'],
            defaults={
